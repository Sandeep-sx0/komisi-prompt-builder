    ...
    <DashboardLayout activeItem="My Links" userType="creator">
      <div className="px-8 py-8 max-w-[1200px]">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">My Links & Assets</h1>
        ...
        <div className="mb-8">
          ...
          <select className="h-11 w-full max-w-xs px-4 text-sm bg-card border border-border outline-none focus:border-foreground">
            <option>MindfulApp</option>
            <option>FocusTimer</option>
            <option>DailyYoga</option>
          </select>
        </div>

        {/* Links */}
        <div className="bg-card border border-border p-6 mb-8">
          <h2 className="text-base font-semibold text-foreground mb-4">Your Affiliate Links</h2>
          <div className="space-y-3">
            {links.map((l) => (
              <div key={l.name}>
                <label className="block text-xs text-text-tertiary mb-1">{l.name}</label>
                <div className="flex items-center gap-2 bg-background-subtle px-4 py-3">
        ...
        {/* Referral Code */}
        <div className="bg-card border border-border p-6 mb-8">
          <h2 className="text-base font-semibold text-foreground mb-4">Your Referral Code</h2>
          <div className="text-center py-4">
            {codeEditing ? (
              <div className="flex items-center justify-center gap-2">
                <input value={refCode} onChange={e => setRefCode(e.target.value.toUpperCase())} className="text-2xl font-semibold tracking-widest font-mono text-center w-40 h-12 border border-border outline-none focus:border-foreground" />
                <Button size="sm" onClick={() => setCodeEditing(false)}>Save</Button>
              </div>
            ) : (
              <>
                <div className="text-2xl font-semibold tracking-[0.1em] font-mono text-foreground">{refCode}</div>
        ...
        <div className="bg-card border border-border p-6">
          <h2 className="text-base font-semibold text-foreground mb-4">QR Code Generator</h2>
          <div className="flex gap-8">
            ...
            <div className="flex-1 space-y-4">
              <div>
                ...
                <select value={qrLink} onChange={e => setQrLink(e.target.value)} className="w-full h-10 px-3 text-sm bg-card border border-border outline-none">
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-card border border-border p-6 shadow-lg z-50 animate-scale-in">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold text-foreground">Create Custom Link</h2>
              <button onClick={() => setCreateOpen(false)} className="text-text-tertiary hover:text-foreground"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <div><label className="block text-sm font-medium text-foreground mb-1.5">Link Name</label><input placeholder='e.g., "TikTok Bio Spring"' className="w-full h-10 px-3 text-sm bg-card border border-border outline-none focus:border-foreground" /></div>
              <div>
              ...
              <div><label className="block text-sm font-medium text-foreground mb-1.5">Custom UTM Source (optional)</label><input placeholder="tiktok-bio" className="w-full h-10 px-3 text-sm bg-card border border-border outline-none focus:border-foreground" /></div>
